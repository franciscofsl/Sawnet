...
using Sawnet.Blazor.Services.LocalStorage.Exceptions;

namespace Sawnet.Blazor.Services.LocalStorage;

internal class BrowserStorageProvider : IStorageProvider
{
    private const string StorageNotAvailableMessage =
    ...
    public async ValueTask<IEnumerable<string>> KeysAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            return await _jSRuntime.InvokeAsync<IEnumerable<string>>("eval", cancellationToken,
    ...
    public void RemoveItems(IEnumerable<string> keys)
    {
        ...
        try
        {
            foreach (var key in keys) _jSInProcessRuntime.InvokeVoid("localStorage.removeItem", key);
        }
        catch (Exception exception)
        {
            if (IsStorageDisabledException(exception))
